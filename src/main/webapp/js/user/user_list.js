...
function eventInit(){
	...
	$('body').on('click', '#btn_query',function(e){
		$('#massage').bootstrapTable('refresh',{url:path+'/post/UserController/queryUserList'});
	});
}
var TableInit = function (){
	...
	oTableInit.Init = function () {
		$('#massage').bootstrapTable({
			url: path+'/post/UserController/queryUserList',             //请求后台的URL（*） 
			method: 'post',                     //请求方式（*）
...
window.EvenInit = {
		'click .RoleOfA': function (e, value, row, index) { // 删除一个分类
				qiao.bs.confirm("确定删除该用户么？",function(){
					...
					AjaxPostUtil.request({url:path+"/post/UserController/queryUserList",params:params,type:'json',callback:function(json){
